Contributing to Vertcore
=======

We're working hard to make *vertcore* the most powerful JavaScript library for working with vertcoin. Our goal is to have *vertcore* be a library that can be used by anyone interested in vertcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in vertcore that any change must adhere.
...
When possible, vertcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Vertcore methods. For example, calling `JSON.stringify` on an Vertcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'vertcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the vertcore object.

DO:
```javascript
var vertcore = require('../');
var PublicKey = vertcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add vertcoin git@github.com:vertcoin-project/vertcore.git
git pull --rebase vertcoin master
```
...
```
Finally go to [github.com/vertcoin-project/vertcore](https://github.com/vertcoin-project/vertcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of vertcore.

If you have any questions feel free to post them to
[github.com/vertcoin-project/vertcore/issues](https://github.com/vertcoin-project/vertcore/issues).
