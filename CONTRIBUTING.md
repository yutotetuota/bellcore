...
We're working hard to make *bellcore* the most powerful JavaScript library for working with bellcoin. Our goal is to have *bellcore* be a library that can be used by anyone interested in bellcoin, and to level expertise differences with great design and documentation.
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Bellcore methods. For example, calling `JSON.stringify` on an Bellcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
[github.com/yutotetuota/bellcore/issues](https://github.com/yutotetuota/bellcore/issues).
